...
public class SubjectKindDeserializer extends StdDeserializer<SubjectKind> { 

    public SubjectKindDeserializer() { 
        this(null); 
    } 

    public SubjectKindDeserializer(Class<?> vc) { 
        super(vc); 
    }

    @Override
    public SubjectKind deserialize(JsonParser jp, DeserializationContext ctxt)
        ...
        SubjectKind src = registry.getSubjectKind(srcStr);

        JsonParser parser = node.findValue("uri").traverse();
        parser.setCodec(jp.getCodec());
        URI uri = parser.readValueAs(URI.class);
        
        SubjectKind kind = registry.getSubjectKind(uri);
        kind.setSource(src);
