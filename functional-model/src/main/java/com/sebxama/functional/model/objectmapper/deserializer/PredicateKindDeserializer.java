...
public class PredicateKindDeserializer extends StdDeserializer<PredicateKind> { 

    public PredicateKindDeserializer() { 
        this(null); 
    } 

    public PredicateKindDeserializer(Class<?> vc) { 
        super(vc); 
    }

    @Override
    public PredicateKind deserialize(JsonParser jp, DeserializationContext ctxt)
        ...
        PredicateKind src = registry.getPredicateKind(srcStr);

        JsonParser parser = node.findValue("uri").traverse();
        parser.setCodec(jp.getCodec());
        URI uri = parser.readValueAs(URI.class);
        
        PredicateKind kind = registry.getPredicateKind(uri);
        kind.setSource(src);
